...
class BorrowerLoanController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('tax_form_image')) $loan->tax_form_image = $request->file('tax_form_image')->store('loan_documents', 'public');
        if ($request->hasFile('household_chart_image')) $loan->household_chart_image = $request->file('household_chart_image')->store('loan_documents', 'public');
        if ($request->hasFile('nrc_front_image')) $loan->nrc_front_image = $request->file('nrc_front_image')->store('loan_documents', 'public');
        if ($request->hasFile('nrc_back_image')) $loan->nrc_back_image = $request->file('nrc_back_image')->store('loan_documents', 'public');
    ...
    public function history()
    {
        ...
        // ယခု Borrower တင်ထားသမျှ ချေးငွေမှတ်တမ်းအားလုံးကို ယူခြင်း
        $loans = \App\Models\BorrowerLoan::where('borrower_id', $borrower_id)
